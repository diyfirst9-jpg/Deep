...
import com.firstt175.deepdrop.prefs.PostProcessingConfig
import java.io.File
...
class SettingsDrawerOverlay(
    ...
    private fun buildPanel(): View {
        ...
        // ---- Post Processing --------------------------------------------------
        val ppSection = collapsibleSection(panel, "POST-PROCESSING")
        var pp = initial.postProcessing
        fun applyP(newValue: PostProcessingConfig) {
            pp = newValue
            prefs.setPostProcessing(newValue)
            runCatching {
                NativeBridge.setPostProcessing(
                    newValue.upscale, newValue.sharpen, newValue.colorPreset,
                    newValue.saturation, newValue.contrast, newValue.brightness, newValue.gamma,
                    newValue.temperature, newValue.exposure, newValue.highlights, newValue.shadows,
                    newValue.vibrance, newValue.imageQuality, newValue.toneMapping,
                )
            }
        }
        ppSection.addView(miniHeader("Upscale"))
        ppSection.addView(chipRow(listOf(
            0 to "Off", 1 to "Bilinear", 2 to "Bicubic", 3 to "Lanczos", 4 to "FSR 1", 5 to "NIS"
        ), pp.upscale) { applyP(pp.copy(upscale = it)) })
        ppSection.addView(miniHeader("Sharpen"))
        ppSection.addView(chipRow(listOf(
            0 to "Off", 1 to "CAS", 2 to "Unsharp", 3 to "RCAS"
        ), pp.sharpen) { applyP(pp.copy(sharpen = it)) })
        ppSection.addView(miniHeader("Color preset"))
        ppSection.addView(chipRow(listOf(
            0 to "Off", 1 to "Natural", 2 to "Vibrant", 3 to "Cinematic",
            4 to "Warm", 5 to "Cool", 6 to "High Contrast"
        ), pp.colorPreset) { applyP(pp.copy(colorPreset = it)) })
        ppSection.addView(miniHeader("Custom color"))

        fun ppSlider(label: String, value: Float, min: Float, max: Float, steps: Int, set: (Float) -> PostProcessingConfig) {
            ...
            ppSection.addView(sliderRow(label, valueView))
            ppSection.addView(SeekBar(ctx).apply {
                this.max = steps
                progress = (((value - min) / (max - min)) * steps).toInt().coerceIn(0, steps)
                progressDrawable = buildSeekTrack()
                thumb = buildSeekThumb()
                splitTrack = false
                wireLiveDrag(
                    label = label,
                    toValue = { pos -> min + (max - min) * (pos.toFloat() / steps.toFloat()) },
                    display = { v -> valueView.text = "%.2f".format(v) },
                    commit = { v -> applyP(set(v)) },
                )
            })
        }
        ppSlider("Saturation", pp.saturation, 0f, 2f, 40) { pp.copy(saturation = it) }
        ppSlider("Contrast", pp.contrast, 0f, 2f, 40) { pp.copy(contrast = it) }
        ppSlider("Brightness", pp.brightness, -1f, 1f, 40) { pp.copy(brightness = it) }
        ppSlider("Gamma", pp.gamma, 0.5f, 2f, 30) { pp.copy(gamma = it) }
        ppSlider("Temperature", pp.temperature, -1f, 1f, 40) { pp.copy(temperature = it) }
        ppSlider("Exposure", pp.exposure, -2f, 2f, 40) { pp.copy(exposure = it) }
        ppSlider("Highlights", pp.highlights, -1f, 1f, 40) { pp.copy(highlights = it) }
        ppSlider("Shadows", pp.shadows, -1f, 1f, 40) { pp.copy(shadows = it) }
        ppSlider("Vibrance", pp.vibrance, -1f, 1f, 40) { pp.copy(vibrance = it) }

        ppSection.addView(miniHeader("Image quality"))
        ppSection.addView(chipRow(listOf(
            0 to "Off", 1 to "Deband", 2 to "Denoise", 3 to "FXAA", 4 to "SMAA"
        ), pp.imageQuality) { applyP(pp.copy(imageQuality = it)) })
        ppSection.addView(miniHeader("HDR / Tone Mapping"))
        ppSection.addView(chipRow(listOf(
            0 to "Off", 1 to "Auto", 2 to "Reinhard", 3 to "ACES", 4 to "Hable"
        ), pp.toneMapping) { applyP(pp.copy(toneMapping = it)) })
