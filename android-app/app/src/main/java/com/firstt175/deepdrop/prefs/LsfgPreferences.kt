...
/** Post-processing controls applied to each presented frame. Values are hot-applied to native. */
data class PostProcessingConfig(
    val upscale: Int = 0,          // 0 Off, 1 Bilinear, 2 Bicubic, 3 Lanczos, 4 FSR1, 5 NIS
    val sharpen: Int = 0,          // 0 Off, 1 CAS, 2 Unsharp Mask, 3 RCAS
    val colorPreset: Int = 0,      // 0 Off, 1 Natural, 2 Vibrant, 3 Cinematic, 4 Warm, 5 Cool, 6 High Contrast
    ...
    val imageQuality: Int = 0,     // 0 Off, 1 Deband, 2 Denoise, 3 FXAA, 4 SMAA
    val toneMapping: Int = 0,      // 0 Off, 1 Auto, 2 Reinhard, 3 ACES, 4 Hable
)
...
class LsfgPreferences(ctx: Context) {
    ...
    fun load(): LsfgConfig = LsfgConfig(
        ...
        postProcessing = PostProcessingConfig(
            upscale = prefs.getInt(KEY_PP_UPSCALE, 0),
            sharpen = prefs.getInt(KEY_PP_SHARPEN, 0),
            colorPreset = prefs.getInt(KEY_PP_COLOR_PRESET, 0),
            saturation = prefs.getFloat(KEY_PP_SATURATION, 1f),
            contrast = prefs.getFloat(KEY_PP_CONTRAST, 1f),
            brightness = prefs.getFloat(KEY_PP_BRIGHTNESS, 0f),
            gamma = prefs.getFloat(KEY_PP_GAMMA, 1f),
            temperature = prefs.getFloat(KEY_PP_TEMPERATURE, 0f),
            exposure = prefs.getFloat(KEY_PP_EXPOSURE, 0f),
            highlights = prefs.getFloat(KEY_PP_HIGHLIGHTS, 0f),
            shadows = prefs.getFloat(KEY_PP_SHADOWS, 0f),
            vibrance = prefs.getFloat(KEY_PP_VIBRANCE, 0f),
            imageQuality = prefs.getInt(KEY_PP_QUALITY, 0),
            toneMapping = prefs.getInt(KEY_PP_TONE, 0),
        ),
    )
    ...
    fun setPostProcessing(value: PostProcessingConfig) {
        prefs.edit()
            .putInt(KEY_PP_UPSCALE, value.upscale)
            .putInt(KEY_PP_SHARPEN, value.sharpen)
            .putInt(KEY_PP_COLOR_PRESET, value.colorPreset)
            .putFloat(KEY_PP_SATURATION, value.saturation)
            .putFloat(KEY_PP_CONTRAST, value.contrast)
            .putFloat(KEY_PP_BRIGHTNESS, value.brightness)
            .putFloat(KEY_PP_GAMMA, value.gamma)
            .putFloat(KEY_PP_TEMPERATURE, value.temperature)
            .putFloat(KEY_PP_EXPOSURE, value.exposure)
            .putFloat(KEY_PP_HIGHLIGHTS, value.highlights)
            .putFloat(KEY_PP_SHADOWS, value.shadows)
            .putFloat(KEY_PP_VIBRANCE, value.vibrance)
            .putInt(KEY_PP_QUALITY, value.imageQuality)
            .putInt(KEY_PP_TONE, value.toneMapping)
            .apply()
    }
    ...
    companion object {
        ...
        private const val KEY_PP_UPSCALE = "pp_upscale"
        private const val KEY_PP_SHARPEN = "pp_sharpen"
        private const val KEY_PP_COLOR_PRESET = "pp_color_preset"
        ...
        private const val KEY_PP_TEMPERATURE = "pp_temperature"
        private const val KEY_PP_EXPOSURE = "pp_exposure"
        private const val KEY_PP_HIGHLIGHTS = "pp_highlights"
        private const val KEY_PP_SHADOWS = "pp_shadows"
        private const val KEY_PP_VIBRANCE = "pp_vibrance"
        private const val KEY_PP_QUALITY = "pp_quality"
        private const val KEY_PP_TONE = "pp_tone"
