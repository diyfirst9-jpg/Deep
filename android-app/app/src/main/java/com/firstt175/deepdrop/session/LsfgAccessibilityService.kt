...
class LsfgAccessibilityService : AccessibilityService() {
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        // type with packageName = the keyboard app, not the game.
        if (isInputMethodPackage(pkg)) {
            // Tell the live overlay (if any) to get out of the way — see
            // OverlayManager.setKeyboardOpen() for why this has to be a full
            // detach rather than a resize/relayout. Do NOT forward this to
            // AutoOverlayController: without this check it reads a package
            // change here as "the target left foreground" and stops the whole
            // session (see onForegroundPackage's sessionActive branch), which
            // is what made the overlay disappear entirely whenever the
            // keyboard showed up.
            OverlayManager.activeInstance?.setKeyboardOpen(true)
            return
        }
        // Any other top-level window becoming foreground means the keyboard
        // (if it was open) is no longer covering the screen — safe to
        // reattach the overlay before evaluating the new foreground package.
        OverlayManager.activeInstance?.setKeyboardOpen(false)
