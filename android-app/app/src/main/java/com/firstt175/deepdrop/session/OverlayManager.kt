...
class OverlayManager(private val ctx: Context) {
     ...
     * IME window, so even though the real keyboard does open underneath (the
     * game/keyboard layout correctly reserves the space for it — that's the
     * empty gap), our window is what's actually being composited on top of that
     * region, and it has nothing there to draw beyond old/blank frame content —
     * the keyboard never becomes visible.
     *
     * Fix: watch for the keyboard opening via the classic visible-display-frame
     * heuristic, and when it does, shrink the WINDOW itself (not the rendered
     * frame — see the fixed-size TextureView in [show]) so our surface simply
     * doesn't extend into that region any more. With nothing of ours covering
     * it, whatever the system composites there next (the real IME) shows
     * through normally, regardless of window-type z-order.
     */
    ...
    private fun applyImeWindowHeight(keyboardVisible: Boolean, visibleBottom: Int) {
        ...
        if (!r.isAttachedToWindow) return
        val lp = r.layoutParams as? WindowManager.LayoutParams ?: return
        lp.height = if (keyboardVisible) visibleBottom.coerceIn(1, overlayHeight) else overlayHeight
        runCatching { wm.updateViewLayout(r, lp) }
            .onFailure { Log.w(TAG, "IME-aware updateViewLayout failed", it) }
        Log.i(
            TAG,
            if (keyboardVisible) {
                "Keyboard opened — shrinking overlay window to height=${lp.height} so it shows through"
            } else {
                "Keyboard closed — restoring overlay window to full height=$overlayHeight"
            },
        )
    }
