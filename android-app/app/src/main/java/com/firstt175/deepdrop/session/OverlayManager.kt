...
class OverlayManager(private val ctx: Context) {
    ...
    // Keyboard-open handling — see setKeyboardOpen(). Driven externally by
    // LsfgAccessibilityService, not by this window's own insets.
    private var hiddenForKeyboard: Boolean = false
    private var savedParamsForKeyboard: WindowManager.LayoutParams? = null
    ...
    /**
     * Makes the on-screen keyboard actually visible while a frame-gen session is
     * running. Call with `true` when the keyboard opens, `false` when it closes —
     * driven by [LsfgAccessibilityService] detecting the real IME's own
     * TYPE_WINDOW_STATE_CHANGED event (see that class), NOT by inspecting this
     * window's own insets: this window sets FLAG_LAYOUT_NO_LIMITS plus the
     * immersive-fullscreen systemUiVisibility flags specifically so it ignores
     * system-bar insets, which as a side effect also makes it blind to the IME
     * inset — `getWindowVisibleDisplayFrame()` on this window never reflects the
     * keyboard opening, so that can't be used as the trigger.
     *
     * In `trustedOverlay` mode this window is TYPE_ACCESSIBILITY_OVERLAY, which
     * Android's window-manager policy always places above TYPE_INPUT_METHOD by
     * design (accessibility overlays, e.g. screen readers, are meant to stay
     * visible over everything) — there is no LayoutParams flag to move it below
     * the keyboard instead. Resizing/shrinking this window doesn't reliably work
     * either (tried previously): whether the keyboard is even included in what's
     * behind our surface at all depends on the active capture method. The only
     * approach that's certain to work regardless of window type or capture
     * method is to fully detach this window from WindowManager while the
     * keyboard is up, so there is nothing of ours left to draw over it, and
     * reattach with the same params once it closes.
     */
    fun setKeyboardOpen(open: Boolean) {
        val wm = hostWindowManager ?: return
        val r = root ?: return
        if (open == hiddenForKeyboard) return
        if (open) {
            if (r.isAttachedToWindow) {
                (r.layoutParams as? WindowManager.LayoutParams)?.let { savedParamsForKeyboard = it }
                runCatching { wm.removeView(r) }
                    .onFailure { Log.w(TAG, "removeView for keyboard failed", it) }
            }
            hiddenForKeyboard = true
            Log.i(TAG, "Keyboard opened — detached overlay window so it can show")
        } else {
            if (!r.isAttachedToWindow) {
                val lp = savedParamsForKeyboard
                if (lp != null) {
                    runCatching { wm.addView(r, lp) }
                        .onFailure { Log.w(TAG, "addView restoring after keyboard failed", it) }
                }
            }
            hiddenForKeyboard = false
            Log.i(TAG, "Keyboard closed — reattached overlay window")
        }
    }
    ...
    companion object {
        ...
        /**
         * The currently-shown overlay, if any — lets [LsfgAccessibilityService]
         * call [setKeyboardOpen] directly without LsfgForegroundService having to
         * thread a reference through. Set in [show], cleared in [hide].
         */
        @Volatile
        var activeInstance: OverlayManager? = null
            private set
    }
}
