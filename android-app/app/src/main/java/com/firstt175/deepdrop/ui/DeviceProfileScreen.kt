...
private fun captureStaticProfileInfo(context: Context): StaticProfileInfo {
    ...
    val gpuMaxFreqMhz = gpuFreqNode?.let { gpuFreqMhz(it.maxPath, it.alreadyMhz) }
...
private fun captureLiveProfileInfo(
    context: Context,
    ...
    val gpuCurFreqMhz = gpuFreqNode?.let { gpuFreqMhz(it.curPath, it.alreadyMhz) }
...
private data class GpuFreqNode(val curPath: String, val maxPath: String, val alreadyMhz: Boolean)

private val GPU_FREQ_CANDIDATES = listOf(
    // Adreno (Qualcomm), modern kernels — devfreq-based governor.
    GpuFreqNode(
        curPath = "/sys/class/kgsl/kgsl-3d0/devfreq/cur_freq",
        maxPath = "/sys/class/kgsl/kgsl-3d0/devfreq/max_freq",
        alreadyMhz = false,
    ),
    // Adreno (Qualcomm), legacy kernels — direct gpuclk node.
    GpuFreqNode(
        curPath = "/sys/class/kgsl/kgsl-3d0/gpuclk",
        maxPath = "/sys/class/kgsl/kgsl-3d0/max_gpuclk",
        alreadyMhz = false,
    ),
    // Samsung Mali (Exynos) — values here are already in MHz, not Hz.
    GpuFreqNode(
        curPath = "/sys/kernel/gpu/gpu_clock",
        maxPath = "/sys/kernel/gpu/gpu_max_clock",
        alreadyMhz = true,
    ),
)

private fun findGpuFreqNode(): GpuFreqNode? {
    GPU_FREQ_CANDIDATES.firstOrNull { readLongFile(it.curPath) != null }?.let { return it }
    ...
    for (dir in dirs) {
        ...
        if (readLongFile(curPath) != null) {
            return GpuFreqNode(curPath, File(dir, "max_freq").absolutePath, alreadyMhz = false)
        }
    }
    return null
}

private fun gpuFreqMhz(path: String, alreadyMhz: Boolean): Int? {
    val raw = readLongFile(path) ?: return null
    return if (alreadyMhz) raw.toInt() else (raw / 1_000_000L).toInt()
}
