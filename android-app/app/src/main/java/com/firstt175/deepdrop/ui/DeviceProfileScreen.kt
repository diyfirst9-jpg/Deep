...
fun DeviceProfileScreen(nav: NavHostController) {
    ...
    Column(
        ...
        LsfgCard {
            SectionHeader(eyebrow = stringResource(R.string.profile_section_battery))
            Spacer(Modifier.height(10.dp))
            val bat = live?.battery
            ProfileRow(
                stringResource(R.string.profile_battery_percent),
                bat?.percent?.let { "$it%" } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(stringResource(R.string.profile_battery_status), bat?.statusLabel ?: "…")
            ProfileRow(stringResource(R.string.profile_battery_plugged), bat?.pluggedLabel ?: "…")
            Spacer(Modifier.height(6.dp))
            ProfileRow(
                stringResource(R.string.profile_battery_charge_now),
                bat?.chargeNowMah?.let { "$it mAh" } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(
                stringResource(R.string.profile_battery_charge_full),
                bat?.chargeFullMah?.let { "$it mAh" } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(
                stringResource(R.string.profile_battery_charge_design),
                bat?.chargeFullDesignMah?.let { "$it mAh" } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(
                stringResource(R.string.profile_battery_health_percent),
                bat?.healthPercent?.let { "%.1f%%".format(it) } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(
                stringResource(R.string.profile_battery_cycles),
                bat?.cycleCount?.let { "$it" } ?: stringResource(R.string.profile_value_na),
            )
            Spacer(Modifier.height(6.dp))
            ProfileRow(
                stringResource(R.string.profile_battery_voltage),
                bat?.voltageNowMv?.let { "%.3f V".format(it / 1000f) } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(
                stringResource(R.string.profile_battery_current),
                bat?.currentNowMa?.let { "$it mA" } ?: stringResource(R.string.profile_value_na),
            )
            bat?.currentSource?.let { ProfileRow(stringResource(R.string.profile_battery_current_source), it) }
            ProfileRow(
                stringResource(R.string.profile_battery_power),
                bat?.powerNowW?.let { "%.2f W".format(it) } ?: stringResource(R.string.profile_value_na),
            )
            Spacer(Modifier.height(6.dp))
            ProfileRow(stringResource(R.string.profile_battery_bypass), bat?.bypassLabel ?: "…")
            bat?.bypassSource?.let { ProfileRow(stringResource(R.string.profile_battery_bypass_source), it) }
            Spacer(Modifier.height(6.dp))
            ProfileRow(
                stringResource(R.string.profile_battery_temp),
                bat?.temperatureC?.let { "%.1f°C".format(it) } ?: stringResource(R.string.profile_value_na),
            )
            ProfileRow(stringResource(R.string.profile_battery_health), bat?.healthLabel ?: "…")
            ProfileRow(stringResource(R.string.profile_battery_technology), bat?.technology ?: stringResource(R.string.profile_value_na))
        }
...
// There's no public Android API to enumerate NPUs (NNAPI's device list is
// an NDK-only C API), so this is heuristic: probe for vendor driver
// libraries / device nodes that only exist when an NPU (and its userspace
// stack) is actually present, then fall back to a known-SoC lookup table
// for detail (core count / TOPS) when the probe can't say more than "yes".
...
// Vendor userspace libraries whose presence implies a matching NPU/DSP:
// - Qualcomm: Hexagon DSP (cdsprpc = compute DSP remote proc call) and the
//   dedicated NPU HAL used on newer Snapdragons.
// - Samsung Exynos: on-chip NPU HAL.
// - MediaTek: APU (AI Processing Unit) MDLA/VPU driver.
// - Google Tensor: uses the Edge TPU stack (darwinn).
private val NPU_LIBRARY_CANDIDATES = listOf(
    "/vendor/lib64/libcdsprpc.so" to ("Qualcomm" to "Hexagon DSP"),
    "/vendor/lib64/libhexagon_nn_skel.so" to ("Qualcomm" to "Hexagon NN"),
    "/vendor/lib64/libSNPE.so" to ("Qualcomm" to "Snapdragon Neural Processing Engine"),
    "/vendor/lib64/hw/android.hardware.neuralnetworks@1.0-service-qti.so" to ("Qualcomm" to "NNAPI HAL"),
    "/vendor/lib64/libnpu_driver.so" to ("Samsung" to "Exynos NPU"),
    "/vendor/lib64/hw/npu.default.so" to ("Samsung" to "Exynos NPU HAL"),
    "/vendor/lib64/libapummdla.so" to ("MediaTek" to "APU (MDLA)"),
    "/vendor/lib64/libapuwareutils.so" to ("MediaTek" to "APU"),
    "/vendor/lib64/libvpu.so" to ("MediaTek" to "APU (VPU)"),
    "/vendor/lib64/libdarwinn_driver.so" to ("Google" to "Edge TPU"),
)

// Character-device nodes as a second, independent signal (some vendors
// ship the driver lib under a different name per firmware branch but the
// device node is more stable).
private val NPU_DEVICE_NODE_CANDIDATES = listOf(
    "/dev/hexagon" to ("Qualcomm" to "Hexagon DSP"),
    "/dev/adsprpc-smd" to ("Qualcomm" to "Hexagon DSP"),
    "/dev/npu" to ("Samsung" to "Exynos NPU"),
    "/dev/mali_npu" to ("Samsung" to "Exynos NPU"),
    "/dev/apusys" to ("MediaTek" to "APU"),
    "/dev/mdla0" to ("MediaTek" to "APU (MDLA)"),
    "/dev/edgetpu" to ("Google" to "Edge TPU"),
)

// devfreq nodes report a live clock, so a matching name here is strong
// evidence — reuses the same scan approach as findGpuFreqNode()/
// readGpuUtilizationPercent() above.
private fun findNpuDevfreqName(): String? {
    val dirs = runCatching { File("/sys/class/devfreq").listFiles() }.getOrNull() ?: return null
    for (dir in dirs) {
        val name = runCatching { File(dir, "name").readText().trim() }.getOrNull() ?: dir.name
        val lower = name.lowercase()
        if (lower.contains("npu") || lower.contains("dsp") || lower.contains("apu") ||
            lower.contains("hexagon") || lower.contains("mdla") || lower.contains("edgetpu")
        ) {
            return name
        }
    }
...
// Fallback detail when a probe found *something* present but couldn't name
// it specifically, or when nothing was found on the filesystem at all but
// the SoC is known to ship an NPU (some OEMs sandbox vendor paths so app
// processes can't stat them, even though the NPU is physically there).
private fun knownSocNpuDetail(soc: String): Pair<String, String>? {
    val s = soc.lowercase()
    return when {
        s.contains("s5e8535") || s.contains("exynos 1380") ->
            "Samsung" to "Exynos NPU (dual-core, ~14.7 TOPS)"
        s.contains("s5e9945") || s.contains("exynos 2400") ->
            "Samsung" to "Exynos NPU (~17.8 TOPS)"
        s.contains("s5e8825") || s.contains("exynos 1280") ->
            "Samsung" to "Exynos NPU (dual-core)"
        s.contains("sm8650") || s.contains("8 gen 3") ->
            "Qualcomm" to "Hexagon NPU (~45 TOPS)"
        s.contains("sm8550") || s.contains("8 gen 2") ->
            "Qualcomm" to "Hexagon NPU"
        s.contains("sm8450") || s.contains("8 gen 1") ->
            "Qualcomm" to "Hexagon NPU"
        s.contains("mt6989") || s.contains("dimensity 9300") ->
            "MediaTek" to "APU 790"
        s.contains("mt6985") || s.contains("dimensity 9200") ->
            "MediaTek" to "APU 6.0"
        s.contains("tensor") ->
            "Google" to "Edge TPU"
        else -> null
...
private fun detectNpu(soc: String): NpuInfo {
    for ((path, meta) in NPU_LIBRARY_CANDIDATES) {
        if (runCatching { File(path).exists() }.getOrDefault(false)) {
            val (vendor, name) = meta
            return NpuInfo(present = true, name = name, detail = vendor, source = "vendor library")
        }
    }
    for ((path, meta) in NPU_DEVICE_NODE_CANDIDATES) {
        if (runCatching { File(path).exists() }.getOrDefault(false)) {
            val (vendor, name) = meta
            return NpuInfo(present = true, name = name, detail = vendor, source = "device node")
        }
    }
    findNpuDevfreqName()?.let { name ->
        return NpuInfo(present = true, name = name, detail = null, source = "devfreq")
    }
    knownSocNpuDetail(soc)?.let { (vendor, name) ->
        return NpuInfo(present = true, name = name, detail = vendor, source = "known chipset")
    }
    return NpuInfo(present = false, name = "N/A", detail = null, source = null)
}
...
private fun captureBatteryInfo(context: Context): BatteryInfo {
    ...
    // Current: BatteryManager.BATTERY_PROPERTY_CURRENT_NOW is µA per the
    // AOSP contract, though a handful of OEM kernels misreport this in mA.
    // We label which source produced the number instead of silently
    // normalizing, since silently "fixing" the units would itself be an
    // unverified guess.
    ...
    val currentNowMa: Int?
    val currentSource: String?
    if (currentFromProperty != null) {
        currentNowMa = currentFromProperty / 1000
        currentSource = "BatteryManager.CURRENT_NOW (µA)"
    } else {
        val sysfsCurrent = readSysfsLong(supplyDir, "current_now")
        currentNowMa = sysfsCurrent?.let { (it / 1000).toInt() }
        currentSource = if (sysfsCurrent != null) "sysfs current_now (µA)" else null
    }

    // Power = V × I, computed only when both operands were themselves
    // directly read (not derived/estimated) — this is a straight physical
    // formula on two real measurements, not a guess.
    val powerNowW = if (voltageNowMv != null && currentNowMa != null) {
        kotlin.math.abs((voltageNowMv / 1000f) * (currentNowMa / 1000f))
    } else {
        null
    }
