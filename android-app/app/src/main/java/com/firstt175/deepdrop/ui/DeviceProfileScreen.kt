...
private suspend fun captureLiveProfileInfo(
    ...
    val cpuLoadPercent = CpuLoadTracker.sampleWithShizukuFallback()
...
private object CpuLoadTracker {
    ...
    private fun readLocalLine(): String? =
        runCatching { File("/proc/stat").bufferedReader().use { it.readLine() } }.getOrNull()

    fun sample(): Float? = parseAndTrack(readLocalLine())

    /**
     * Some OEM builds (seen on Samsung/Android 15) deny app-level reads of
     * /proc/stat outright — readLocalLine() returns null forever, which is
     * what showed as permanent "N/A" CPU load. If Shizuku is connected, its
     * shell has broader access than the app sandbox, so fall back to that
     * before giving up.
     */
    suspend fun sampleWithShizukuFallback(): Float? {
        parseAndTrack(readLocalLine())?.let { return it }
        if (!ShizukuDisplayPermission.isShizukuAvailable()) return null
        val line = ShizukuDisplayPermission.exec("cat /proc/stat")
            ?.takeIf { it.ok }
            ?.stdout
            ?.lineSequence()
            ?.firstOrNull { it.trim().startsWith("cpu ") }
 ...
 * from app-level reads (thermal_zone*/type and .../temp both quietly return
 * nothing rather than throwing), fall back to Shizuku's shell if it's
