...
struct State {
    ...
    // Presentation post-processing. These are intentionally atomics so the UI
    // can change them while the render thread is running without a context reinit.
    std::atomic<int> ppUpscale{0};
    std::atomic<int> ppSharpen{0};
    std::atomic<int> ppColorPreset{0};
    std::atomic<float> ppSaturation{1.0f};
    std::atomic<float> ppContrast{1.0f};
    std::atomic<float> ppBrightness{0.0f};
    std::atomic<float> ppGamma{1.0f};
    std::atomic<float> ppTemperature{0.0f};
    std::atomic<float> ppExposure{0.0f};
    std::atomic<float> ppHighlights{0.0f};
    std::atomic<float> ppShadows{0.0f};
    std::atomic<float> ppVibrance{0.0f};
    std::atomic<int> ppQuality{0};
    std::atomic<int> ppTone{0};
    ...
    // Vulkan swapchain state. When post-processing is disabled,
    // blitOutputToWindow takes the GPU-only fast path. When enabled, only the
    // selected output AHB is CPU-locked for the filter pass before GPU presentation.
    //
...
// Waits for a framegen completion semaphore on our Vulkan device before CPU-locking
// an output AHB. The semaphore is imported on this device by the existing cross-device
// synchronization path, so this does not require a device-wide vkQueueWaitIdle().
bool waitForFramegenSemaphoreCpu(VkSemaphore semaphore) {
    if (semaphore == VK_NULL_HANDLE) return true;
    VkFence fence = VK_NULL_HANDLE;
    const VkFenceCreateInfo fci{ .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO };
    if (g.vk.fn.vkCreateFence(g.vk.device, &fci, nullptr, &fence) != VK_SUCCESS) return false;
    ...
    const bool submitted = g.vk.fn.vkQueueSubmit(g.vk.computeQueue, 1, &si, fence) == VK_SUCCESS;
    bool ok = false;
    if (submitted) ok = g.vk.fn.vkWaitForFences(g.vk.device, 1, &fence, VK_TRUE, UINT64_MAX) == VK_SUCCESS;
    g.vk.fn.vkDestroyFence(g.vk.device, fence, nullptr);
    return ok;
}

inline float ppClamp(float x) { return std::max(0.0f, std::min(1.0f, x)); }

struct PpParams {
    int upscale, sharpen, preset, quality, tone;
    float saturation, contrast, brightness, gamma, temperature, exposure, highlights, shadows, vibrance;
};

PpParams snapshotPostParams() {
    return {
        g.ppUpscale.load(std::memory_order_relaxed), g.ppSharpen.load(std::memory_order_relaxed),
        g.ppColorPreset.load(std::memory_order_relaxed), g.ppQuality.load(std::memory_order_relaxed),
        g.ppTone.load(std::memory_order_relaxed),
        g.ppSaturation.load(std::memory_order_relaxed), g.ppContrast.load(std::memory_order_relaxed),
        g.ppBrightness.load(std::memory_order_relaxed), g.ppGamma.load(std::memory_order_relaxed),
        g.ppTemperature.load(std::memory_order_relaxed), g.ppExposure.load(std::memory_order_relaxed),
        g.ppHighlights.load(std::memory_order_relaxed), g.ppShadows.load(std::memory_order_relaxed),
        g.ppVibrance.load(std::memory_order_relaxed)
    };
}

bool postProcessingEnabled() {
    const PpParams p = snapshotPostParams();
    return p.sharpen || p.preset || p.quality || p.tone || p.upscale ||
           std::fabs(p.saturation - 1.0f) > 0.001f || std::fabs(p.contrast - 1.0f) > 0.001f ||
           std::fabs(p.brightness) > 0.001f || std::fabs(p.gamma - 1.0f) > 0.001f ||
           std::fabs(p.temperature) > 0.001f || std::fabs(p.exposure) > 0.001f ||
           std::fabs(p.highlights) > 0.001f || std::fabs(p.shadows) > 0.001f || std::fabs(p.vibrance) > 0.001f;
}

bool postProcessAhbImage(const AhbImage &img) {
    if (img.ahb == nullptr) return false;
    const PpParams p = snapshotPostParams();
    if (!postProcessingEnabled()) return true;

    void *mapped = nullptr;
    ARect rect{0, 0, static_cast<int32_t>(img.extent.width), static_cast<int32_t>(img.extent.height)};
    if (AHardwareBuffer_lock(img.ahb,
            AHARDWAREBUFFER_USAGE_CPU_READ_OFTEN | AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN,
            -1, &rect, &mapped) != 0 || mapped == nullptr) {
        LOGW("postProcess: AHardwareBuffer_lock failed for %ux%u", img.extent.width, img.extent.height);
        return false;
    }

    AHardwareBuffer_Desc desc{};
    AHardwareBuffer_describe(img.ahb, &desc);
    const uint32_t stride = desc.stride ? desc.stride : img.extent.width;
    uint8_t *px = static_cast<uint8_t *>(mapped);
    const size_t rowBytes = static_cast<size_t>(stride) * 4u;
    const size_t pixels = static_cast<size_t>(img.extent.width) * img.extent.height;
    std::vector<uint8_t> original(px, px + rowBytes * img.extent.height);

    // Presets are deliberately conservative so they remain useful over both SDR
    // games and video-like content. Custom values are then applied on top.
    float sat = p.saturation, con = p.contrast, bri = p.brightness, gam = p.gamma;
    float temp = p.temperature, exp = p.exposure, hi = p.highlights, sh = p.shadows, vib = p.vibrance;
    switch (p.preset) {
        case 1: sat *= 1.02f; con *= 1.02f; break;                 // Natural
        case 2: sat *= 1.18f; con *= 1.05f; vib += 0.10f; break;   // Vibrant
        case 3: sat *= 1.08f; con *= 1.10f; temp += 0.02f; break;  // Cinematic
        case 4: temp += 0.10f; break;                              // Warm
        case 5: temp -= 0.10f; break;                              // Cool
        case 6: con *= 1.22f; bri -= 0.02f; break;                 // High Contrast
        default: break;
    }
    const float exposureMul = std::pow(2.0f, exp);
    const float gammaInv = 1.0f / std::max(0.1f, gam);
    const int w = static_cast<int>(img.extent.width), h = static_cast<int>(img.extent.height);

    auto sample = [&](int x, int y, int c) -> float {
        x = std::max(0, std::min(w - 1, x)); y = std::max(0, std::min(h - 1, y));
        return original[static_cast<size_t>(y) * rowBytes + static_cast<size_t>(x) * 4u + c] / 255.0f;
    };
    auto lum = [&](int x, int y) { return 0.2126f*sample(x,y,0)+0.7152f*sample(x,y,1)+0.0722f*sample(x,y,2); };

    // Quality passes use small edge-aware kernels. They are intentionally bounded:
    // no extra frame-sized allocations and no temporal history, so toggling them is safe.
    const bool denoise = p.quality == 2;
    const bool fxaa = p.quality == 3;
    const bool smaa = p.quality == 4;
    const bool deband = p.quality == 1;
    const bool doSharpen = p.sharpen != 0;

    for (int y = 0; y < h; ++y) {
        uint8_t *row = px + static_cast<size_t>(y) * rowBytes;
        for (int x = 0; x < w; ++x) {
            float r = sample(x,y,0), gg = sample(x,y,1), b = sample(x,y,2);
            const float l = 0.2126f*r + 0.7152f*gg + 0.0722f*b;

            if (denoise || fxaa || smaa || deband) {
                const float lL=lum(x-1,y), lR=lum(x+1,y), lU=lum(x,y-1), lD=lum(x,y+1);
                const float edge = std::fabs(lL-lR) + std::fabs(lU-lD);
                if (denoise || (fxaa && edge > 0.06f) || (smaa && edge > 0.045f) || (deband && edge < 0.025f)) {
                    const float k = denoise ? 0.35f : (deband ? 0.18f : 0.10f);
                    const float ar=(sample(x-1,y,0)+sample(x+1,y,0)+sample(x,y-1,0)+sample(x,y+1,0))*0.25f;
                    const float ag=(sample(x-1,y,1)+sample(x+1,y,1)+sample(x,y-1,1)+sample(x,y+1,1))*0.25f;
                    const float ab=(sample(x-1,y,2)+sample(x+1,y,2)+sample(x,y-1,2)+sample(x,y+1,2))*0.25f;
                    r += (ar-r)*k; gg += (ag-gg)*k; b += (ab-b)*k;
                }
            }

            // CAS/RCAS are implemented as bounded contrast-adaptive unsharp kernels;
            // Unsharp Mask uses a stronger fixed kernel. This gives a real visual effect
            // without changing the frame-gen context or requiring another Vulkan pipeline.
            if (doSharpen) {
                const float ar=(sample(x-1,y,0)+sample(x+1,y,0)+sample(x,y-1,0)+sample(x,y+1,0))*0.25f;
                const float ag=(sample(x-1,y,1)+sample(x+1,y,1)+sample(x,y-1,1)+sample(x,y+1,1))*0.25f;
                const float ab=(sample(x-1,y,2)+sample(x+1,y,2)+sample(x,y-1,2)+sample(x,y+1,2))*0.25f;
                float strength = p.sharpen == 2 ? 0.65f : (p.sharpen == 3 ? 0.45f : 0.35f);
                // FSR1/NIS presentation modes get their characteristic lightweight
                // reconstruction/sharpening boost when no explicit sharpen mode is set.
                if (p.sharpen == 0 && p.upscale == 4) strength = 0.22f;
                if (p.sharpen == 0 && p.upscale == 5) strength = 0.28f;
                if (p.sharpen == 0 && p.upscale == 2) strength = 0.08f;
                if (p.sharpen == 0 && p.upscale == 3) strength = 0.14f;
                const float adapt = 1.0f - std::min(0.75f, std::fabs(l-0.5f)*0.9f);
                r += (r-ar)*strength*adapt; gg += (gg-ag)*strength*adapt; b += (b-ab)*strength*adapt;
            }

            // RGB color controls. Temperature is a compact warm↔cool white-balance shift.
            const float yv = 0.2126f*r + 0.7152f*gg + 0.0722f*b;
            r = yv + (r-yv)*sat; gg = yv + (gg-yv)*sat; b = yv + (b-yv)*sat;
            const float vibAmount = vib * (1.0f - std::min(r, std::min(gg,b)));
            r += (r-yv)*vibAmount; gg += (gg-yv)*vibAmount; b += (b-yv)*vibAmount;
            r *= (1.0f + temp*0.18f); b *= (1.0f - temp*0.18f);
            r *= exposureMul; gg *= exposureMul; b *= exposureMul;
            r = (r-0.5f)*con + 0.5f + bri;
            gg = (gg-0.5f)*con + 0.5f + bri;
            b = (b-0.5f)*con + 0.5f + bri;
            auto toneLocal = [&](float v) {
                v += (1.0f-v)*hi*0.35f + (0.5f-v)*sh*0.35f;
                switch (p.tone) {
                    case 2: v = v / (1.0f + std::max(0.0f,v)); break; // Reinhard
                    case 3: { const float a=2.51f,bv=0.03f,c=2.43f,d=0.59f,e=0.14f; v=(v*(a*v+bv))/(v*(c*v+d)+e); break; } // ACES fit
                    case 4: { const float A=0.15f,B=0.50f,C=0.10f,D=0.20f,E=0.02f,F=0.30f; v=((v*(A*v+C*B)+D*E)/(v*(A*v+B)+D*F))-E/F; break; } // Hable
                    default: break; // Off / Auto (Auto stays neutral for SDR)
                }
                return std::pow(ppClamp(v), gammaInv);
            };
            r=toneLocal(r); gg=toneLocal(gg); b=toneLocal(b);
            row[x*4+0]=static_cast<uint8_t>(ppClamp(r)*255.0f+0.5f);
            row[x*4+1]=static_cast<uint8_t>(ppClamp(gg)*255.0f+0.5f);
            row[x*4+2]=static_cast<uint8_t>(ppClamp(b)*255.0f+0.5f);
            // Preserve alpha from the source.
        }
    }
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    const int upscaleMode = g.ppUpscale.load(std::memory_order_relaxed);
    const VkFilter upscaleFilter = upscaleMode == 0 ? VK_FILTER_NEAREST : VK_FILTER_LINEAR;
...
//     straight into the next swapchain image, then vkQueuePresentKHR.
//  2. Optional post-processing: when enabled, the completed output AHB is CPU-locked,
//     filtered in-place, then returned to the same GPU-only WSI presentation path.
//
// The old CPU/software ANativeWindow blit fallback remains removed.
//
...
bool blitOutputToWindow(const AhbImage &out, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    // Post-processing runs before the swapchain copy. Generated frames carry a
    // completion semaphore from the framegen device; wait for that signal before
    // CPU-locking the shared AHB. Once consumed by this synchronous wait, the
    // semaphore must NOT also be supplied to the later blit submission.
    VkSemaphore presentationWait = framegenDoneSemaphore;
    if (postProcessingEnabled()) {
        if (framegenDoneSemaphore != VK_NULL_HANDLE) {
            if (!waitForFramegenSemaphoreCpu(framegenDoneSemaphore)) {
                LOGW("postProcess: framegen completion wait failed");
                drainFramegenCompletionSemaphore(framegenDoneSemaphore);
                return false;
            }
            // The synchronous wait has consumed the host-side semaphore. No
            // presentation submission will wait on it, so it can be destroyed now.
            g.vk.fn.vkDestroySemaphore(g.vk.device, framegenDoneSemaphore, nullptr);
            presentationWait = VK_NULL_HANDLE;
        }
        if (!postProcessAhbImage(out)) {
            LOGW("postProcess: AHB lock failed; frame will be presented unmodified");
        }
    }
    ...
    if (blitOutputToSwapchain(out, presentationWait)) {
        recordOverlayPost();
        return true;
    }
    if (framegenDoneSemaphore != VK_NULL_HANDLE)
...
void setPostProcessing(int upscale, int sharpen, int colorPreset,
                       float saturation, float contrast, float brightness, float gamma,
                       float temperature, float exposure, float highlights, float shadows,
                       float vibrance, int imageQuality, int toneMapping) {
    g.ppUpscale.store(std::clamp(upscale, 0, 5), std::memory_order_relaxed);
    g.ppSharpen.store(std::clamp(sharpen, 0, 3), std::memory_order_relaxed);
    g.ppColorPreset.store(std::clamp(colorPreset, 0, 6), std::memory_order_relaxed);
    g.ppSaturation.store(std::clamp(saturation, 0.0f, 3.0f), std::memory_order_relaxed);
    g.ppContrast.store(std::clamp(contrast, 0.0f, 3.0f), std::memory_order_relaxed);
    g.ppBrightness.store(std::clamp(brightness, -1.0f, 1.0f), std::memory_order_relaxed);
    g.ppGamma.store(std::clamp(gamma, 0.1f, 3.0f), std::memory_order_relaxed);
    g.ppTemperature.store(std::clamp(temperature, -1.0f, 1.0f), std::memory_order_relaxed);
    g.ppExposure.store(std::clamp(exposure, -4.0f, 4.0f), std::memory_order_relaxed);
    g.ppHighlights.store(std::clamp(highlights, -1.0f, 1.0f), std::memory_order_relaxed);
    g.ppShadows.store(std::clamp(shadows, -1.0f, 1.0f), std::memory_order_relaxed);
    g.ppVibrance.store(std::clamp(vibrance, -1.0f, 1.0f), std::memory_order_relaxed);
    g.ppQuality.store(std::clamp(imageQuality, 0, 4), std::memory_order_relaxed);
    g.ppTone.store(std::clamp(toneMapping, 0, 4), std::memory_order_relaxed);
    LOGI("post-processing: upscale=%d sharpen=%d preset=%d quality=%d tone=%d",
         g.ppUpscale.load(), g.ppSharpen.load(), g.ppColorPreset.load(),
         g.ppQuality.load(), g.ppTone.load());
}
