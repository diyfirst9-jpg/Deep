...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    if (cfg.aiBackend) {
        ...
        if (cfg.aiEngine == 1) {
            ...
            rc = g.aiIfrnet->load(cfg.aiModelDir, false, /*vulkanDeviceIndex*/ -1, ncnnCpuThreadCount());
        } else {
            g.ai = new NcnnInterpolator();
            rc = g.ai->load(cfg.aiModelDir, false, /*vulkanDeviceIndex*/ -1, ncnnCpuThreadCount());
        }
        if (rc == kNcnnOk) {
            g.aiLoaded = true;
