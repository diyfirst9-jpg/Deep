...
// Generated by CMake (EmbedSpirv.cmake) from shaders/upscale_sharpen.comp —
// see CMakeLists.txt's upscale_sharpen_shader custom target.
#include "upscale_sharpen_spv.h"
...
struct State {
    ...
    struct SwapchainState {
        ...
        // Whether the surface/device actually supports STORAGE_BIT on the
        // swapchain images. Probed once in createSwapchain(); the compute
        // upscale-sharpen path is unavailable (falls back to plain
        // vkCmdBlitImage) whenever this is false, so toggling the feature on
        // can never break presentation on a device that doesn't support it.
        bool storageCapable = false;
    } swap;

    // CAS-lite upscale+sharpen compute pass, fused into blitOutputToSwapchain
    // (see upscale_sharpen.comp). Lazily created on first use under
    // g.presentMu; torn down in shutdownRenderLoop()/destroySwapchain().
    struct UpscaleSharpen {
        std::atomic<bool> enabled{false};     // toggled via setUpscaleSharpenEnabled()
        float amount = 0.35f;                 // 0..1, same range as the existing CAS post-process
        ...
        // Output AHBs are a small fixed set reused in place (see comment on
        // runAiInterpolate), so caching sampled views by VkImage handle is
        // safe for the process lifetime — never invalidated mid-session.
        std::unordered_map<VkImage, VkImageView> srcViewCache;
        // Parallel to g.swap.images; rebuilt whenever the swapchain is.
        std::vector<VkImageView> dstStorageViews;
...
// Forward declarations — defined further down (near blitOutputToSwapchain)
// but needed here because createSwapchain()/destroySwapchain() must keep
// the compute upscale-sharpen resources (per-image storage views +
// descriptor sets) in sync with the swapchain image set.
void destroyUpscaleSwapchainViews();
bool ensureUpscaleSwapchainViews();
...
void destroySwapchain() {
    ...
    // Views wrap g.swap.images, which we're about to clear — must go first.
    destroyUpscaleSwapchainViews();
...
bool createSwapchain() {
    ...
    // STORAGE_BIT is opportunistic, not mandatory: plenty of Android
    // compositors don't advertise it on the presentable format, and unlike
    // TRANSFER_DST above we must not fail swapchain creation over it — we
    // just lose the compute upscale-sharpen path (see UpscaleSharpen) and
    // fall back to the existing vkCmdBlitImage scaled path.
    g.swap.storageCapable = (caps.supportedUsageFlags & VK_IMAGE_USAGE_STORAGE_BIT) != 0;
    VkImageUsageFlags imageUsage = VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    if (g.swap.storageCapable) imageUsage |= VK_IMAGE_USAGE_STORAGE_BIT;
    ...
    // Best-effort: failure here just means the upscale-sharpen toggle stays
    // inert (blitOutputToSwapchain falls back to vkCmdBlitImage) — it must
    // never fail swapchain creation itself.
    if (g.swap.storageCapable && !ensureUpscaleSwapchainViews()) {
        LOGW("ensureUpscaleSwapchainViews failed; upscale-sharpen toggle will be a no-op this session");
    }
    return true;
}
...
// ---------------------------------------------------------------------------
// Compute upscale-sharpen pass (upscale_sharpen.comp). Lazily created; all
// entry points here are only ever called from blitOutputToWindow's call
// path, i.e. under g.presentMu — no separate locking needed.
// ---------------------------------------------------------------------------

// One-time pipeline/sampler/pool creation. Cheap to call repeatedly —
// no-ops once g.upscale.pipelineReady is set. Process-lifetime resources;
// never torn down until shutdownRenderLoop().
bool ensureUpscalePipeline() {
    if (g.upscale.pipelineReady) return true;
    if (g.vk.device == VK_NULL_HANDLE) return false;

    const VkShaderModuleCreateInfo smci{
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
        .codeSize = kUpscaleSharpenSpv_size,
        .pCode = kUpscaleSharpenSpv,
    };
    VkShaderModule module = VK_NULL_HANDLE;
    if (g.vk.fn.vkCreateShaderModule(g.vk.device, &smci, nullptr, &module) != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreateShaderModule failed");
        return false;
    }

    const VkSamplerCreateInfo sampInfo{
    ...
    if (g.vk.fn.vkCreateSampler(g.vk.device, &sampInfo, nullptr, &g.upscale.sampler) != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreateSampler failed");
        g.vk.fn.vkDestroyShaderModule(g.vk.device, module, nullptr);
        return false;
    }
    ...
    if (g.vk.fn.vkCreateDescriptorSetLayout(g.vk.device, &dslci, nullptr,
            &g.upscale.descSetLayout) != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreateDescriptorSetLayout failed");
        g.vk.fn.vkDestroyShaderModule(g.vk.device, module, nullptr);
        return false;
    }

    const VkPushConstantRange pcRange{
        .stageFlags = VK_SHADER_STAGE_COMPUTE_BIT, .offset = 0, .size = 5 * sizeof(float),
    };
    const VkPipelineLayoutCreateInfo plci{
        .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
        .setLayoutCount = 1, .pSetLayouts = &g.upscale.descSetLayout,
        .pushConstantRangeCount = 1, .pPushConstantRanges = &pcRange,
    };
    if (g.vk.fn.vkCreatePipelineLayout(g.vk.device, &plci, nullptr,
            &g.upscale.pipelineLayout) != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreatePipelineLayout failed");
        g.vk.fn.vkDestroyShaderModule(g.vk.device, module, nullptr);
        return false;
    }
    ...
    const VkComputePipelineCreateInfo cpci{
        ...
        .layout = g.upscale.pipelineLayout,
    };
    const VkResult pr = g.vk.fn.vkCreateComputePipelines(g.vk.device, VK_NULL_HANDLE, 1, &cpci,
        nullptr, &g.upscale.pipeline);
    g.vk.fn.vkDestroyShaderModule(g.vk.device, module, nullptr); // not needed after pipeline creation
    if (pr != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreateComputePipelines failed (%d)", (int)pr);
        return false;
    }

    // Sized generously (32 sets) — comfortably covers any realistic
    // swapchain image count (typically 2-4) with headroom for a resize.
    ...
    if (g.vk.fn.vkCreateDescriptorPool(g.vk.device, &dpci, nullptr, &g.upscale.descPool) != VK_SUCCESS) {
        LOGW("ensureUpscalePipeline: vkCreateDescriptorPool failed");
        return false;
    }

    g.upscale.pipelineReady = true;
    return true;
}

// Sampled VkImageView for a framegen output AHB image, cached by VkImage
// handle (see UpscaleSharpen::srcViewCache comment — the small set of output
// AHBs is reused in place for the whole session, so this cache never goes
// stale).
VkImageView getOrCreateUpscaleSrcView(const AhbImage &src) {
    auto it = g.upscale.srcViewCache.find(src.image);
    if (it != g.upscale.srcViewCache.end()) return it->second;
    ...
    if (g.vk.fn.vkCreateImageView(g.vk.device, &ivci, nullptr, &view) != VK_SUCCESS) {
        LOGW("getOrCreateUpscaleSrcView: vkCreateImageView failed");
        return VK_NULL_HANDLE;
    }
    g.upscale.srcViewCache.emplace(src.image, view);
    return view;
}

// (Re)builds the per-swapchain-image storage views + descriptor sets.
// Called once after every successful createSwapchain(). The descriptor
// sets' image-0 (sampled) binding is left unwritten here — it's rewritten
// per-dispatch in blitOutputToSwapchain since the source AHB rotates frame
// to frame; only the dst (binding 1) half is fixed per swapchain image and
// written once here.
bool ensureUpscaleSwapchainViews() {
    if (!ensureUpscalePipeline()) return false;
    destroyUpscaleSwapchainViews();

    g.upscale.dstStorageViews.resize(g.swap.images.size(), VK_NULL_HANDLE);
    g.upscale.descSets.resize(g.swap.images.size(), VK_NULL_HANDLE);

    std::vector<VkDescriptorSetLayout> layouts(g.swap.images.size(), g.upscale.descSetLayout);
    if (!layouts.empty()) {
        const VkDescriptorSetAllocateInfo dsai{
            .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
            .descriptorPool = g.upscale.descPool,
            .descriptorSetCount = static_cast<uint32_t>(layouts.size()),
            .pSetLayouts = layouts.data(),
        };
        if (g.vk.fn.vkAllocateDescriptorSets(g.vk.device, &dsai, g.upscale.descSets.data()) != VK_SUCCESS) {
            LOGW("ensureUpscaleSwapchainViews: vkAllocateDescriptorSets failed");
            return false;
        }
    }

    for (size_t i = 0; i < g.swap.images.size(); ++i) {
        const VkImageViewCreateInfo ivci{
            .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
            .image = g.swap.images[i],
            .viewType = VK_IMAGE_VIEW_TYPE_2D,
            .format = g.swap.format,
            .subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 },
        };
        if (g.vk.fn.vkCreateImageView(g.vk.device, &ivci, nullptr,
                &g.upscale.dstStorageViews[i]) != VK_SUCCESS) {
            LOGW("ensureUpscaleSwapchainViews: vkCreateImageView (dst %zu) failed", i);
            return false;
        }
        const VkDescriptorImageInfo dstInfo{
            .imageView = g.upscale.dstStorageViews[i],
            .imageLayout = VK_IMAGE_LAYOUT_GENERAL,
        };
        const VkWriteDescriptorSet write{
            .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
            .dstSet = g.upscale.descSets[i],
            .dstBinding = 1,
            .descriptorCount = 1,
            .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
            .pImageInfo = &dstInfo,
        };
        g.vk.fn.vkUpdateDescriptorSets(g.vk.device, 1, &write, 0, nullptr);
    }
    return true;
}

void destroyUpscaleSwapchainViews() {
    if (g.vk.device != VK_NULL_HANDLE && g.vk.fn.vkDestroyImageView != nullptr) {
        for (VkImageView v : g.upscale.dstStorageViews) {
            if (v != VK_NULL_HANDLE) g.vk.fn.vkDestroyImageView(g.vk.device, v, nullptr);
        }
    }
    g.upscale.dstStorageViews.clear();
    // Descriptor sets are freed implicitly by vkDestroyDescriptorPool at
    // shutdown, or explicitly here if the pool supports FREE_DESCRIPTOR_SET
    // and is still alive (swapchain resize case).
    if (g.vk.device != VK_NULL_HANDLE && g.vk.fn.vkFreeDescriptorSets != nullptr &&
            g.upscale.descPool != VK_NULL_HANDLE && !g.upscale.descSets.empty()) {
        g.vk.fn.vkFreeDescriptorSets(g.vk.device, g.upscale.descPool,
            static_cast<uint32_t>(g.upscale.descSets.size()), g.upscale.descSets.data());
    }
    g.upscale.descSets.clear();
}

void destroyUpscalePipeline() {
    if (g.vk.device == VK_NULL_HANDLE) return;
    destroyUpscaleSwapchainViews();
    for (auto &[image, view] : g.upscale.srcViewCache) {
        if (view != VK_NULL_HANDLE && g.vk.fn.vkDestroyImageView != nullptr)
            g.vk.fn.vkDestroyImageView(g.vk.device, view, nullptr);
    }
    g.upscale.srcViewCache.clear();
    if (g.upscale.descPool != VK_NULL_HANDLE && g.vk.fn.vkDestroyDescriptorPool != nullptr)
        g.vk.fn.vkDestroyDescriptorPool(g.vk.device, g.upscale.descPool, nullptr);
    if (g.upscale.pipeline != VK_NULL_HANDLE && g.vk.fn.vkDestroyPipeline != nullptr)
        g.vk.fn.vkDestroyPipeline(g.vk.device, g.upscale.pipeline, nullptr);
    if (g.upscale.pipelineLayout != VK_NULL_HANDLE && g.vk.fn.vkDestroyPipelineLayout != nullptr)
        g.vk.fn.vkDestroyPipelineLayout(g.vk.device, g.upscale.pipelineLayout, nullptr);
    if (g.upscale.descSetLayout != VK_NULL_HANDLE && g.vk.fn.vkDestroyDescriptorSetLayout != nullptr)
        g.vk.fn.vkDestroyDescriptorSetLayout(g.vk.device, g.upscale.descSetLayout, nullptr);
    if (g.upscale.sampler != VK_NULL_HANDLE && g.vk.fn.vkDestroySampler != nullptr)
        g.vk.fn.vkDestroySampler(g.vk.device, g.upscale.sampler, nullptr);
    g.upscale = State::UpscaleSharpen{};
}

// Public toggle, called from JNI. Lock-free — read on the hot path via
// std::memory_order_relaxed in blitOutputToSwapchain.
void setUpscaleSharpenEnabled(bool enabled) {
    g.upscale.enabled.store(enabled, std::memory_order_relaxed);
}

void setUpscaleSharpenAmount(float amount) {
    g.upscale.amount = clamp01(amount);
}
...
bool blitOutputToSwapchain(const AhbImage &src, VkSemaphore framegenDoneSemaphore = VK_NULL_HANDLE) {
    ...
    // Whether this frame takes the compute upscale-sharpen path instead of
    // the plain blit: only when actually scaled (1:1 stays on the
    // vkCmdCopyImage fast-path below — sharpening a non-upscaled frame isn't
    // what the toggle is for), the surface/device support STORAGE_BIT on the
    // swapchain image, and the pipeline is actually ready. Any of these
    // being false falls straight back to the existing behavior, so flipping
    // the toggle can never break presentation.
    const bool isScaled = src.extent.width != g.swap.extent.width ||
                           src.extent.height != g.swap.extent.height;
    const bool useCompute = isScaled && g.swap.storageCapable &&
        g.upscale.enabled.load(std::memory_order_relaxed) &&
        imageIdx < g.upscale.descSets.size() &&
        ensureUpscalePipeline();
    VkImageView upscaleSrcView = VK_NULL_HANDLE;
    if (useCompute) {
        upscaleSrcView = getOrCreateUpscaleSrcView(src);
        if (upscaleSrcView == VK_NULL_HANDLE) {
            // View creation failed (shouldn't happen once cached, but the
            // first hit per AHB can) — fall back to the blit this frame
            // rather than fail the whole present.
        }
    }
    const bool doCompute = useCompute && upscaleSrcView != VK_NULL_HANDLE;

    // Source: AHB-backed image owned by framegen's device between uses.
    // Acquire for TRANSFER_READ (blit path) or SHADER_READ (compute path),
    // release back to EXTERNAL after.
    const uint32_t foreign = VK_QUEUE_FAMILY_EXTERNAL;
    const VkImageLayout srcWorkLayout = doCompute
        ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
    const VkAccessFlags srcWorkAccess = doCompute
        ? VK_ACCESS_SHADER_READ_BIT : VK_ACCESS_TRANSFER_READ_BIT;
    VkImageMemoryBarrier srcAcquire = makeImageBarrier(src.image,
        0, srcWorkAccess,
        VK_IMAGE_LAYOUT_UNDEFINED, srcWorkLayout,
    ...
    const VkImageLayout dstWorkLayout = doCompute
        ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    const VkAccessFlags dstWorkAccess = doCompute
    ...
    const VkPipelineStageFlags preDstStage = doCompute
        ? VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT : VK_PIPELINE_STAGE_TRANSFER_BIT;
    g.vk.fn.vkCmdPipelineBarrier(cb,
        VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, preDstStage,
    ...
    if (doCompute) {
        if (!loggedBlitMode.exchange(true, std::memory_order_relaxed)) {
            LOGW("blitOutputToSwapchain: Scaled output (%ux%u -> %ux%u), upscale-sharpen ON. "
                 "Using compute pass (fused into this submit, no extra sync).",
                 src.extent.width, src.extent.height, g.swap.extent.width, g.swap.extent.height);
        }
        VkDescriptorSet ds = g.upscale.descSets[imageIdx];
        const VkDescriptorImageInfo srcInfo{
            .sampler = g.upscale.sampler,
            .imageView = upscaleSrcView,
        ...
        g.vk.fn.vkCmdBindPipeline(cb, VK_PIPELINE_BIND_POINT_COMPUTE, g.upscale.pipeline);
        g.vk.fn.vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_COMPUTE,
            g.upscale.pipelineLayout, 0, 1, &ds, 0, nullptr);
        const float pc[5] = {
            1.0f / static_cast<float>(src.extent.width),
            1.0f / static_cast<float>(src.extent.height),
            static_cast<float>(g.swap.extent.width),
            static_cast<float>(g.swap.extent.height),
            g.upscale.amount,
        };
        g.vk.fn.vkCmdPushConstants(cb, g.upscale.pipelineLayout,
            VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc), pc);
        const uint32_t groupsX = (g.swap.extent.width + 7) / 8;
        const uint32_t groupsY = (g.swap.extent.height + 7) / 8;
        g.vk.fn.vkCmdDispatch(cb, groupsX, groupsY, 1);
    } else if (src.extent.width == g.swap.extent.width &&
        src.extent.height == g.swap.extent.height) {
        if (!loggedBlitMode.exchange(true, std::memory_order_relaxed)) {
    ...
    VkImageMemoryBarrier srcRelease = makeImageBarrier(src.image,
        srcWorkAccess, 0,
        srcWorkLayout, VK_IMAGE_LAYOUT_GENERAL,
        g.vk.computeFamilyIdx, foreign);
    VkImageMemoryBarrier dstToPresent = makeImageBarrier(g.swap.images[imageIdx],
        dstWorkAccess, 0,
        dstWorkLayout, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
        VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED);
    VkImageMemoryBarrier post[2] = {srcRelease, dstToPresent};
    const VkPipelineStageFlags postSrcStage = doCompute
        ? VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT : VK_PIPELINE_STAGE_TRANSFER_BIT;
    g.vk.fn.vkCmdPipelineBarrier(cb,
        postSrcStage, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
...
void shutdownRenderLoop() {
...

    {
        ...
        g.swapWinW = 0;
        g.swapWinH = 0;
        // Process-lifetime compute upscale-sharpen resources (pipeline,
        // sampler, cached src views) — must go before destroy_session()
        // tears down g.vk.device out from under it.
